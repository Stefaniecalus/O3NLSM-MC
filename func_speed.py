...
def flux_side(lat_coords, spinvalues, side, nref):
    ...
    # Recalculate flux for each edge, but avoid re-indexing or repeated calculations
    for i in range(side_length):
        flux += gauge_pot(lat_coords, spinvalues, side[i], side[(i+1) % side_length], nref)
...
def flux_cube(lat_coords, spinvalues, indices, nref):
    ...
    flux = 0
    # Instead of calling .values() multiple times, precompute it once
    side_list = list(sides.values())
    
    # Sum flux for each side
...
def get_cone(lat_coords, spinvalues, flipcoord, nref, L, eps=0.01):
    ...
    if np.isnan(A).any():
        dx, dy = random.choice([-eps, 0, eps]), random.choice([-eps, 0, eps])
